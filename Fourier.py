...
'''acumulados=np.zeros((n),dtype=np.complex)
for i in range(n):
    for j in range(n):
        acumulados[i]=acumulados[i]+datosignal[j]*np.exp(-2j*np.pi*j*i/n)'''
...
'''
def pasarbajos(freq,fftx,filtro):
...
fc1=1000
fftbajos=pasarbajos(ffreq,transf,fc1)
...
def intercuadrat(xx,yy):
    ...
    return icuadrat
'''
interplineal=interlineal(ffreq,datosincom)
interpcuad=intercuadrat(ffreq,datosincom)'''
